...
//
// Deploy to: mobile_app/app/api/ai/facebook-post-content/route.ts
...
function computeSEOScore(params: {
  ...
  const nameCount = (
    content.match(
      new RegExp(businessName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi"),
    ) ?? []
  ).length;
  if (nameCount === 1) {
    score += 15;
  } else if (nameCount === 0) {
    tips.push("Mention your business name once");
  } else if (nameCount > 2) {
    tips.push("Business name mentioned too many times — feels spammy");
  } else {
    score += 8;
  }
  ...
  if (titleInOpening) {
    ...
    tips.push("Include your post topic in the opening sentence");
  }
  ...
  if (hashtags.length >= 3 && hashtags.length <= 6) {
    ...
    score += 8;
    tips.push("Use 3–5 hashtags for best discoverability");
  } else {
  ...
  if (hasCTA) {
    ...
    tips.push("Add a clear call-to-action (e.g. 'Visit us today')");
  }

  // 5. Has emojis (10pts)
  const emojiCount = (content.match(/\p{Emoji}/gu) ?? []).filter(
    (e) => e !== "#",
  ).length;
  if (emojiCount >= 2 && emojiCount <= 5) {
    ...
    score += 5;
    tips.push("Add 2–4 emojis to boost engagement");
  } else if (emojiCount > 5) {
    score += 5;
    tips.push("Reduce emojis — 2–4 is optimal");
  } else {
    tips.push("Add 2–4 emojis to improve click-through rate");
  }
  ...
  if (wordCount >= 50 && wordCount <= 200) {
    ...
    tips.push("Post is too short — aim for 60–150 words");
  } else {
    tips.push("Post is very long — consider trimming to under 200 words");
  }
  ...
  if (!spammy) {
    ...
    tips.push("Some words are repeated too often — vary your language");
  }
