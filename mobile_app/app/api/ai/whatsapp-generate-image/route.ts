// WhatsApp image generation — square 1080×1080, suitable for WhatsApp sharing & status
// Dual-provider strategy: Pollinations (free) → Together AI (fallback)
...
function buildImagePrompt(params: {
  ...
  const { topic, businessName, businessCategory, style } = params;
  ...
  return `
    eye-catching promotional image for a WhatsApp business message from "${businessName}",
    ${category} brand,
    topic: ${cleanTopic || "business promotion"},

    visual composition:
    visually dominant subject, rule-of-thirds or centred composition,
    clean uncluttered background that supports the subject,
    optimised for square WhatsApp sharing, highly shareable social content,
    professional commercial quality, attention-grabbing at small sizes,

    style: ${styleMod},

    text rule (critical): ${textRule},
    prefer clean visuals with no text overlay — let the image speak for itself,

    format: 1080×1080 square, WhatsApp-compatible social media image,
...
export async function POST(req: Request) {
  try {
    ...
    const prompt = buildImagePrompt({ topic, businessName, businessCategory, style });
    ...
    try {
      ...
      console.warn("[wa-generate-image] pollinations failed:", polErr.message);
      try {
    ...
    console.error("[wa-generate-image] error:", err);
