...
import {
  Plus, Search, RefreshCw, Trash2, Edit3, Calendar, Zap,
  ShoppingBag, Clock, CheckCircle2, Building2, WifiOff,
...
function normalisePost(g: GMBPost): Post {
  return {
    ...
    updateDate: new Date(g.updateTime).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" }),
...
/* ══════════════════════════════════════════════════════════
   API
══════════════════════════════════════════════════════════ */
async function fetchPosts(locationId: string, pageToken?: string): Promise<PostsResponse> {
  const params = new URLSearchParams({ location: `accounts/me/locations/${locationId}` });
...
function typeColor(t: PostTopicType, isDark: boolean) {
  if (t === "EVENT") return isDark ? "bg-purple-500/15 text-purple-400 border-purple-500/20" : "bg-purple-50 text-purple-600 border-purple-200";
  if (t === "OFFER") return isDark ? "bg-orange-500/15 text-orange-400 border-orange-500/20" : "bg-orange-50 text-orange-600 border-orange-200";
  return isDark ? "bg-blue-500/15 text-blue-400 border-blue-500/20" : "bg-blue-50 text-blue-600 border-blue-200";
}

function stateBadge(s: PostState, isDark: boolean) {
  if (s === "LIVE") return { cls: isDark ? "bg-green-500/15 text-green-400" : "bg-green-50 text-green-600", label: "Live" };
  if (s === "REJECTED") return { cls: isDark ? "bg-red-500/15 text-red-400" : "bg-red-50 text-red-600", label: "Rejected" };
  if (s === "PROCESSING") return { cls: isDark ? "bg-yellow-500/15 text-yellow-400" : "bg-yellow-50 text-yellow-600", label: "Processing" };
  return { cls: isDark ? "bg-slate-500/15 text-slate-400" : "bg-slate-50 text-slate-500", label: "Draft" };
}
...
/* ══════════════════════════════════════════════════════════
   DELETE MODAL
══════════════════════════════════════════════════════════ */
function DeleteModal({ post, isDark, onConfirm, onCancel, isDeleting }: {
  post: Post; isDark: boolean; onConfirm: () => void;
  onCancel: () => void; isDeleting: boolean;
}) {
  return (
    <div
    ...
    >
      <div className={`w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl ${isDark ? "bg-[#131c2d]" : "bg-white"}`}>
        <div className="p-6 text-center">
          ...
          <h3 className={`text-[17px] font-black mb-1.5 ${isDark ? "text-white" : "text-slate-900"}`}
            style={{ letterSpacing: "-0.03em" }}>Delete Post?</h3>
          <p className={`text-[13px] leading-relaxed mb-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
            This will permanently remove the post from your Google Business Profile.
          </p>
          <p className={`text-[12px] font-medium line-clamp-1 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
            "{post.summary.slice(0, 60)}{post.summary.length > 60 ? "…" : ""}"
          </p>
        </div>
        <div className={`px-4 pb-5 flex gap-2.5 border-t ${isDark ? "border-white/[0.06]" : "border-slate-100"} pt-4`}>
          ...
          <button onClick={onConfirm} disabled={isDeleting}
            ...
            style={{ background: "linear-gradient(135deg,#dc2626,#ef4444)", boxShadow: "0 4px 14px rgba(220,38,38,0.38)" }}>
            {isDeleting ? <><Spin size={14} white /> Deleting…</> : <><Trash2 size={14} /> Delete</>}
          </button>
        </div>
      </div>
    </div>
...
/* ══════════════════════════════════════════════════════════
   POST CARD
══════════════════════════════════════════════════════════ */
function PostCard({ post, isDark, onDelete, onEdit, isDeleting }: {
  post: Post; isDark: boolean;
  onDelete: (p: Post) => void; onEdit: (p: Post) => void; isDeleting: boolean;
}) {
    ...
    <div className={`rounded-2xl border overflow-hidden transition-all duration-200 flex flex-col
      ...
      {post.imageUrl && (
        <div className="relative h-48 md:h-52 overflow-hidden">
          <Image src={post.imageUrl} alt="Post image" fill className="object-cover" unoptimized />
          <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
          <div className="absolute top-2 right-2">
            <span className={`flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border ${typeColor(post.topicType, isDark)}`}>
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-start gap-3 mb-2.5">
          {!post.imageUrl && (
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border ${typeColor(post.topicType, isDark)}`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5 flex-wrap">
              {!post.imageUrl && (
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${typeColor(post.topicType, isDark)}`}>
                  {post.topicType}
                </span>
              )}
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${cls}`}>{label}</span>
              <span className={`text-[10px] ml-auto shrink-0 ${isDark ? "text-slate-500" : "text-slate-400"}`}>{post.date}</span>
            </div>
            {post.eventTitle && (
              <p className={`text-[12px] font-bold mb-0.5 ${isDark ? "text-blue-400" : "text-blue-600"}`}>
                📅 {post.eventTitle}
              </p>
            )}
          </div>
          ...
          <div className="relative shrink-0">
            ...
            {menuOpen && (
              <>
                ...
                <div className={`absolute right-0 top-8 z-20 rounded-2xl border overflow-hidden shadow-xl min-w-[140px]
        ...
        {/* body */}
        <p className={`text-[13.5px] leading-relaxed flex-1 ${isDark ? "text-slate-300" : "text-slate-700"}`}
          style={{ fontFamily: "-apple-system,'SF Pro Text',sans-serif" }}>
          {needsTrunc && !expanded ? post.summary.slice(0, 100) + "…" : post.summary}
          {needsTrunc && (
        ...
        {/* coupon */}
        {post.couponCode && (
          <div className={`mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold self-start
        ...
        {post.cta && post.cta !== "NONE" && (
          <div className={`mt-2 inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-lg self-start
        ...
        {/* footer */}
        <div className={`flex items-center gap-2 mt-3 pt-3 border-t ${isDark ? "border-white/[0.05]" : "border-slate-100"}`}>
          ...
          <span className={`ml-auto text-[10px] flex items-center gap-1 ${isDark ? "text-slate-600" : "text-slate-400"}`}>
...
function StatCard({ label, value, icon, color, isDark }: {
  label: string; value: string | number; icon: React.ReactNode; color: string; isDark: boolean;
}) {
  return (
    <div className={`rounded-2xl p-3.5 border flex flex-col gap-1.5
      ${isDark ? "bg-[#131c2d] border-white/[0.06]" : "bg-white border-black/[0.05] shadow-sm"}`}>
...
export default function GooglePostsPage() {
  ...
  const visible = posts.filter(p => {
    const matchFilter = filter === "ALL" ? true
      : filter === "LIVE" ? p.state === "LIVE"
      : filter === "REJECTED" ? p.state === "REJECTED"
      : p.topicType === filter;
    ...
    <div className="w-full" style={{ fontFamily: "-apple-system,'SF Pro Text',sans-serif" }}>
      ...
      <div className="pt-2 pb-4 flex items-center justify-between">
        <div>
          ...
          {user?.googleLocationName && (
            <div className="flex items-center gap-1.5">
              ...
              <span className={`text-[12px] ${isDark ? "text-slate-500" : "text-slate-500"}`}>
                {user.googleLocationName}
              </span>
            </div>
          )}
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => router.push("/post/create")}
            ...
            style={{ background: "linear-gradient(135deg,#1d4ed8,#3b82f6)", boxShadow: "0 3px 12px rgba(37,99,235,0.38)" }}>
      ...
      {isInitial && (
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-1">
            {[...Array(4)].map((_, i) => (
              <div key={i} className={`rounded-2xl p-4 h-20 animate-pulse border
                ${isDark ? "bg-[#131c2d] border-white/[0.06]" : "bg-white border-black/[0.05]"}`} />
            ))}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[...Array(4)].map((_, i) => <PostCardSkeleton key={i} isDark={isDark} />)}
          </div>
        </div>
      )}
      ...
      {!userLoading && !user?.googleLocationId && (
        <div className={`rounded-2xl p-8 text-center border
          ...
          <p className={`text-[12.5px] mb-4 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
      ...
      {isError && (
        <div className={`rounded-2xl p-4 flex items-start gap-3 border mb-4
          ...
          <div>
            <p className="text-[13px] font-semibold text-red-400 mb-0.5">Failed to load posts</p>
        ...
        <>
          ...
          {total > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <StatCard label="Total Posts" value={total}     icon={<FileText size={14} />}     color="#3b82f6" isDark={isDark} />
              <StatCard label="Live"        value={liveCount} icon={<CheckCircle2 size={14} />} color="#22c55e" isDark={isDark} />
              <StatCard label="Events"      value={evtCount}  icon={<Calendar size={14} />}     color="#8b5cf6" isDark={isDark} />
              <StatCard label="Offers"      value={ofrCount}  icon={<ShoppingBag size={14} />}  color="#f97316" isDark={isDark} />
            </div>
          )}
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 mb-4">
            {/* search */}
            <div className={`flex items-center gap-2.5 h-[42px] px-3.5 rounded-[13px] border flex-1
          ...
          {posts.length === 0 && !isLoading && (
            <div className={`rounded-2xl p-10 text-center border
              ...
              <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4
                ...
                <FileText size={26} className={isDark ? "text-blue-400" : "text-blue-500"} />
              </div>
              <p className={`text-[15px] font-bold mb-1.5 ${isDark ? "text-white" : "text-slate-900"}`}>No Posts Yet</p>
              <p className={`text-[13px] mb-5 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
              ...
              <button onClick={() => router.push("/post/create")}
                ...
                style={{ background: "linear-gradient(135deg,#1d4ed8,#3b82f6)", boxShadow: "0 3px 14px rgba(37,99,235,0.38)" }}>
                <span className="flex items-center gap-1.5"><Plus size={15} /> Create First Post</span>
              </button>
            </div>
          )}
          ...
          {posts.length > 0 && visible.length === 0 && (
            <div className={`rounded-2xl p-8 text-center border
              ${isDark ? "bg-[#131c2d] border-white/[0.06]" : "bg-white border-black/[0.05]"}`}>
              <Search size={24} className={`mx-auto mb-2 ${isDark ? "text-slate-600" : "text-slate-300"}`} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-start">
              {visible.map(p => (
                <PostCard
                  ...
                  onEdit={p => router.push(`/post/edit/${encodeURIComponent(p.name)}`)}
                  isDeleting={deleteMutation.isPending && deleteMutation.variables === p.name}
                />
              ))}
            </div>
          )}
          ...
          <div className="mt-5">
            ...
            {!loadingMore && !hasMore && total > 0 && (
              <div className="flex items-center gap-3 py-2">
                ...
                <p className={`text-[12px] font-medium ${isDark ? "text-slate-600" : "text-slate-400"}`}>
      ...
      {/* ── TOAST ── */}
      {toastMsg && (
        <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-[300]
