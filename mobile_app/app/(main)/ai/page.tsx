...
const SUGGESTIONS = [
  {
    icon: <Search size={14} />,
    text: "How do I rank #1 on Google for my local business?",
    color: "#22c55e",
  },
  {
    icon: <Megaphone size={14} />,
    text: "Best Google Ads strategy for ₹20,000/month budget",
    color: "#f97316",
  },
  {
    icon: <BarChart3 size={14} />,
    text: "How to get more Google reviews quickly?",
    color: "#3b82f6",
  },
  {
    icon: <TrendingUp size={14} />,
    text: "Instagram growth strategy for 10K followers",
    color: "#ec4899",
  },
  {
    icon: <Star size={14} />,
    text: "Improve my Google Business Profile score",
    color: "#f59e0b",
  },
  {
    icon: <Globe size={14} />,
    text: "Facebook Ads vs Google Ads — which is better?",
    color: "#8b5cf6",
  },
  {
    icon: <Zap size={14} />,
    text: "SEO basics for a beginner small business owner",
    color: "#06b6d4",
  },
  {
    icon: <Target size={14} />,
    text: "Set up Meta retargeting for website visitors",
    color: "#1877F2",
  },
];
...
function MD({ text, streaming }: { text: string; streaming?: boolean }) {
  ...
  return (
    <div className="space-y-1.5 text-[13px] sm:text-[15px] leading-[1.75]">
      {text.split("\n").map((line, i) => {
        if (!line.trim()) return <div key={i} className="h-1.5" />;
        if (line.startsWith("### "))
          return (
            <p key={i} className="font-bold text-[15px] mt-3 mb-0.5">
              {line.slice(4)}
            </p>
          );
        if (line.startsWith("## "))
          return (
            <p key={i} className="font-bold text-[16px] mt-3 mb-0.5">
              {line.slice(3)}
            </p>
          );
        if (line.match(/^[•\-\*]\s/))
        ...
        if (line.match(/^\d+\.\s/)) {
          ...
          return (
            <div key={i} className="flex items-start gap-2.5 ml-1">
              <span className="shrink-0 font-semibold opacity-40 text-[13px] mt-0.5 w-4">
                {num}.
              </span>
              <span className="flex-1">
    ...
    return (
      <div className="flex justify-end px-4 md:px-8 lg:px-12">
        <div className="max-w-[82%] md:max-w-[65%]">
          <div
            className="px-4 py-3 rounded-[22px] rounded-br-[6px] text-[15px] leading-relaxed"
            style={{
              background: isDark
    ...
    >
      <div className="flex gap-3 items-start max-w-3xl">
        ...
        <div className="flex-1 min-w-0 pb-2">
          ...
          {!msg.isStreaming && msg.content && (
            <div
              className={`flex items-center gap-0.5 mt-2.5 transition-opacity duration-150 ${hover ? "opacity-100" : "opacity-0"}`}
            >
              <button
                onClick={copy}
      ...
      try {
        ...
        if (reader) {
          ...
          if (!full) {
            ...
            if (c)
              setMessages((prev) =>
                prev.map((m) => (m.id === aiId ? { ...m, content: c } : m)),
              );
          }
        }
      } catch (e: any) {
  ...
  return (
    ...
    */
    ...
    >
      ...
      >
        ...
        {isEmpty && (
          <div className="flex flex-col items-center justify-center flex-1 px-4 py-6">
            ...
            <h2
              className="text-[22px] font-bold mb-1.5 text-center"
              style={{
                color: textP,
            ...
            <p
              className="text-[14px] text-center mb-7 leading-relaxed"
              style={{ color: textS, maxWidth: 280 }}
            >
              Ask me anything about SEO, Google Ads, Meta Ads, or growing your
              business online.
            </p>
        ...
        <div ref={endRef} className="h-1 mb-25" />
      </div>
      ...
      ══════════════════════════════════════ */}
      <div className="fixed bottom-23 left-0 right-0 md:left-[220px] lg:left-[240px] px-4 z-50">
        ...
        <div className="max-w-2xl mx-auto">
            ...
            <div className="flex items-center justify-between px-3 py-2.5">
              ...
              <div className="flex items-center gap-2">
                {input.length > 0 && !loading && (
                  <span className="text-[11px]" style={{ color: textS }}>
                    {input.length}
                  </span>
                )}
                {loading ? (
                  ...
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: input.trim()
                        ? "0 2px 12px rgba(99,102,241,0.45)"
                        : "none",
                  ...
                  >
                    <Send
                      size={14}
                      style={{ color: input.trim() ? "white" : textS }}
                    />
                  </button>
                )}
              </div>
          ...
          <p
            ...
            style={{
              color: isDark ? "rgba(255,255,255,0.13)" : "rgba(0,0,0,0.22)",
            }}
          >
