...
function InputField({
  ...
  isDark, leftIcon, rightElement, autoComplete,
}: InputFieldProps) {
    ...
    >
      ...
      <div className="flex-1 flex flex-col px-5">
        ...
        <div className="mb-2 text-center">
          ...
          `}
            style={{ letterSpacing: "-0.04em", fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', sans-serif" }}
          >
            Welcome back 👋
          </h1>
        ...
        `}>
          ...
          {/* error */}
          {error && (
            <p className="text-[12px] font-medium text-red-400 -mt-1">{error}</p>
          )}
        ...
        <div className="flex items-center gap-3 my-5">
          <div className={`flex-1 h-px ${isDark ? "bg-white/[0.07]" : "bg-slate-200"}`} />
          <span className={`text-[11px] font-semibold ${isDark ? "text-slate-600" : "text-slate-600"}`}>
            or
          </span>
          <div className={`flex-1 h-px ${isDark ? "bg-white/[0.07]" : "bg-slate-200"}`} />
        </div>
        ...
        {/* sign up */}
        <p className={`text-center text-[13px] mt-6 ${isDark ? "text-slate-500" : "text-slate-600"}`}>
        ...
        {/* terms */}
        <p className={`text-center text-[11px] mt-4 leading-relaxed ${isDark ? "text-slate-700" : "text-slate-500"}`}>
          By signing in you agree to our{" "}
          <Link href="/terms"   className={`underline ${isDark ? "text-slate-500" : "text-slate-600"}`}>Terms</Link>
          {" & "}
          <Link href="/privacy" className={`underline ${isDark ? "text-slate-500" : "text-slate-600"}`}>Privacy Policy</Link>
        </p>
      </div>
